...
| `slack_attachment_text_extraction` | 5 minutes | Extracts searchable text from PDF, DOCX, PPTX, and text attachments. PDFs must already contain embedded text; this workflow does not OCR images. |
...
| `SLACK_ATTACHMENT_EXTRACTION_ENABLED` | `true` | Extracts searchable text from supported Slack attachments when Slack ETL is enabled. |
| `SLACK_ATTACHMENT_EXTRACTION_INTERVAL_SECONDS` | `300` | How often to scan for attachments requiring extraction. |
| `SLACK_ATTACHMENT_EXTRACTION_BATCH_SIZE` | `10` | Maximum attachments processed by one workflow run before it requeues. |
| `SLACK_ATTACHMENT_EXTRACTION_MAX_DOWNLOAD_BYTES` | `52428800` | Maximum stored or transiently fetched attachment size accepted for extraction. |
| `SLACK_ATTACHMENT_EXTRACTION_MAX_EXPANDED_BYTES` | `262144000` | Maximum expanded size accepted for DOCX and PPTX ZIP containers. |
| `SLACK_ATTACHMENT_EXTRACTION_MAX_PAGES` | `250` | Maximum PDF pages processed. Longer PDFs are indexed partially and marked truncated. |
| `SLACK_ATTACHMENT_EXTRACTION_MAX_CHARACTERS` | `2000000` | Maximum extracted characters stored per attachment. |
...
| `slack_attachment_text_extractions` | Final text-extraction outcome and searchable text for each supported attachment. |
...
types, Slack permalinks, download status, checksums, the message the file was
attached to, and successfully extracted text. Oversized supported files can be
fetched transiently for extraction without storing their bytes in Postgres.
Private Slack download URLs are never indexed. Image-only PDFs remain
metadata-only because extraction does not perform OCR.
