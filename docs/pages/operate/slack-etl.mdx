...
attached to, and successfully extracted text. Only attachments downloaded into
Postgres under `SLACK_ETL_ATTACHMENT_MAX_BYTES` are extracted. Private Slack
download URLs are never indexed. Oversized attachments and image-only PDFs
